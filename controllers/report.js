...
exports.updateExcelData = async (req, res) => {
  ...
  jwt.verify(token, process.env.SECRET, async (err) => {
    ...
    await appData.create(
      { ...req.body, member_id: nanoid(5) },
      (err, requests) => {
        if (err)
          return res
            .status(500)
            .send("Some problem occurred while creating your excel");
        res
          .status(200)
          .send(
            "Your request has been sent. One of our team member will contact you shortly"
          );
      }
    );
  });
...
exports.loadPreferenceData = async (req, res) => {
  const { username } = req.body;
  const token = cookieToken(req);
  jwt.verify(token, process.env.SECRET, async (err) => {
    if (err)
      return res
        ...
        .send({ auth: false, message: "Failed to authenticate token." });
        preferences.create(
      { ...req.body, assessment_id: nanoid(5) },
      (err, requests) => {
        if (err)
          return res
            .status(500)
  ...
  jwt.verify(token, process.env.SECRET, (err, decoded) => {
    ...
    Reports.find({ user }, (err, reports) => {
      if (err)
        return res
          .status(500)
          .send({ message: "There was a problem finding the reports" });
      if (reports.length === 0)
        return res.status(204).send("No report present for this user");
